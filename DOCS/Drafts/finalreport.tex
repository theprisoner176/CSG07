...
~\\\\
On the list of walks page, this is where all the walks show up that has been uploaded from the app. This is done by the website connects to the database and receive all the details and echo them on this page. The user can pick what walk he would like to look at. By clicking on the walk he wants to see, that will take him to the walk\_details.php and that shows the walk in more detail.  The list works with the size of the page. The bigger the page, the more columns there are. This is don’t by making the information that is echo to float to the left.
...
\paragraph{Files}
This page only shows what is on the database and cannot change anything in the database.
\paragraph{Interfaces}
...
Not much you can change to affect the web. It doesn’t edit much just displays the information. The only real risk is not connecting to the database and retrieving the information.
\paragraph{Physical Limitations Of The Program}
Again not many limitations on the web site. It is only displaying the information. It works with all browsers.
\paragraph{Rebuilding And Testing}
When rebuilding it, makes sure the CSS files are in the same place so the pages are still directed to the part.  Just make sure if the connection to the database is fine. Just test this by PHPunit. It will show you if you have any errors. 
\newpage
...
\paragraph{Main Data Areas}
There are no substantial data storage areas in the database as the database itself is the storage area. However when we get information from the database it returns it as an array. But this is covered in the Web Maintanence document.
\paragraph{Files}
...
\includepdf[scale=	0.95, pages={1-29}, pagecommand={}]{ProjectPlan.pdf}
\newgeometry{top=1.5cm, bottom =1.2cm}
\includepdf[scale=	0.95, pages={30}, pagecommand={}, landscape=true]{ProjectPlan.pdf}
\restoregeometry
\includepdf[scale=0.95, pages={31-33}, pagecommand={}]{ProjectPlan.pdf}
\subsection{Revised Design Document}
\includepdf[scale=0.95, pages=1-21, pagecommand={}]{DesignSpec.pdf}
\newgeometry{top=1.5cm, bottom =1.2cm}
\includepdf[scale=	0.95, pages={22}, pagecommand={}, landscape=true]{DesignSpec.pdf}
\restoregeometry
\includepdf[scale=	0.95, pages={23-29}, pagecommand={}]{DesignSpec.pdf}
