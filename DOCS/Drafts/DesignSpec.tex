...
\textdollar sql = "INSERT INTO List\_of\_Walks(title, shortDesc, longDesc, hours, distance)
\\\\
\zeroindent	
VALUES('\textdollar title','\textdollar short\_desc', '\textdollar long\_desc', '\textdollar hours', '\textdollar distance')"; 
\\\\
		\textdollar queryWalk = mysqli\_query(\textdollar walk\_conn,\textdollar sql);
        \\\\
        \zeroindent
		\textdollar walkID = mysqli\_insert\_id(\textdollar walk\_conn);	
\\\\
		foreach(\textdollar route as \textdollar loc)\{
        \\\\
			//loop through every location
			\\\\
			\textdollar longitude = \textdollar loc['longitude'];
            \\\\
			\textdollar latitude = \textdollar loc['latitude'];
            \\\\
			\textdollar time = \textdollar loc['time'];
			\\\\
			\textdollar sql = "INSERT INTO Location(walkID, latitude, longitude, timestamp)	
            \\\\
			VALUES('\textdollar walkID', '\textdollar latitude','\textdollar longitude', '\textdollar time')";
            \\\\
		\textdollar queryLoc =	mysqli\_query(\textdollar walk\_conn,\textdollar sql);
        \\\\
			\textdollar locID = mysqli\_insert\_id(\textdollar walk\_conn);
			\\\\
			if(isSet(\textdollar loc['description']))\{
            \\\\
				//If description, location is point of interest
				\textdollar description = \textdollar loc['description'];
				\textdollar name = \textdollar loc['title'];
				\textdollar sql = "INSERT INTO Place\_description(description, locationId, name)
						values('\textdollar description', '\textdollar locID', '\textdollar name')";
				\textdollar queryPlace = mysqli\_query(\textdollar walk\_conn,\textdollar sql);
				\textdollar placeId = mysqli\_insert\_id(\textdollar walk\_conn);
				if(isSet(\textdollar loc['images']))\{
				\textdollar photoCount = 0;
				//If there are images decode and rename them
					foreach(\textdollar loc['images']as \textdollar image)\{
						\textdollar image = implode(\textdollar image);
						\textdollar image = str\_replace("\\", "", \textdollar image, \textdollar count);
						\textdollar image =  base64\_decode(\textdollar image);
						\textdollar photoName = \textdollar walkID . "\_" . \textdollar locID . "\_" . \textdollar placeId . "\_" . \textdollar photoCount;
						file\_put\_contents("images/".\textdollar photoName . ".jpg",\textdollar image);
						\textdollar photoCount++;
						\textdollar sql = "INSERT INTO Photo(photoName, placeId)
						values('\textdollar photoName', '\textdollar placeId')";
						\textdollar queryPhoto =	mysqli\_query(\textdollar walk\_conn,\textdollar sql);
					\}
				\}
			\}
