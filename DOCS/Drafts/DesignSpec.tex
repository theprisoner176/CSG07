...
\lhead{Group Project 07 â€“ Design Specification}
...
This section describes the most significant classes in the application. The complete set of classes can be seen in the class diagram â€“ Section 4.1.2. These classes will all be written in Java.
\paragraph{WalkModel}
\zeroindent
...
The user will also be able to view points of interest on the map. This will serve as a separate file that will interact with Googleâ€™s system.
...
Action 2 is optional for the user, because it is not mandatory to have a Point of interest or photos in every walk. If a user decides to click the â€œAdd POIâ€� button, the LocationListener gives the coordinates of the current location to the RouteRecorder and they are saved in the WalkModel object.
...
\end{landscape}
\restoregeometry
The classes ending in â€˜screenâ€™, are all Activities. They all, in some way, display a layout to the screen and respond to user input. Any response that requires further processing would be passed to another class and then handed back to be displayed, but it would be the â€™screenâ€™ class itself that initialised the action.  
There are several classes that have been suffixed with â€˜Viewâ€™, these classes all extend the android class View. They are all visible to the user and act much like â€˜screenâ€™ classes except that they donâ€™t use the whole screen and do not change the displayed screen only create new Views.  
...
*/  
$con=mysqli_connect("db.dcs.aber.ac.ukâ€�, â€œcsgp07_13_14â€�,"csadmgp07","c54admgp07");
